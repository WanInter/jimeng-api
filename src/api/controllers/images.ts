...
export function getModel(model: string, regionInfo: RegionInfo): ModelResult {
  ...
  } else if (regionInfo.isHK || regionInfo.isJP || regionInfo.isSG || regionInfo.isVN) {
...
function logResolutionInfo(userModel: string, resolution: ResolutionResult, regionInfo: RegionInfo) {
  ...
  if (userModel === 'nanobanana') {
    if (regionInfo.isUS) {
      ...
      const regionName = regionInfo.isHK ? '香港' : regionInfo.isJP ? '日本' : regionInfo.isVN ? '越南' : '新加坡';
 ...
 * @returns 比例字符串，如 "16:9"，未检测到返回 null
 */
export function detectAspectRatioKey(prompt: string): string | null {
  ...
  for (const match of matches) {
    ...
    if (ASPECT_RATIOS.includes(key)) {
      logger.info(`从提示词检测到比例: ${key}`);
      return key;
    }
  }

  if (/横屏|横版|宽屏/.test(prompt)) {
    logger.info(`检测到横屏关键词 → 16:9`);
    return "16:9";
  }
  if (/竖屏|竖版|手机/.test(prompt)) {
    logger.info(`检测到竖屏关键词 → 9:16`);
    return "9:16";
  }
  if (/方形|正方/.test(prompt)) {
    logger.info(`检测到方形关键词 → 1:1`);
    return "1:1";
  }
...
export async function generateImagesWithRetry(
  ...
  // 智能比例检测：如果用户没显式传ratio或传了默认值，从prompt中检测
  let finalRatio = options.ratio || "1:1";
  if (finalRatio === "1:1") {
    const detected = detectAspectRatioKey(prompt);
    if (detected) {
      finalRatio = detected;
      logger.info(`智能比例检测覆盖: ${detected}`);
    }
  }
  ...
  while (currentResIndex < resolutionLevels.length) {
    try {
      ...
      logger.info(`尝试生成图像，分辨率: ${currentOptions.resolution}，比例: ${finalRatio}`);
